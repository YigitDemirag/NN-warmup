...
def setup_logger_kwargs(exp_name, seed=None, data_dir=None, datestamp=False):
    ...
    Args:
        ...
        data_dir (string): Path to folder where results should be saved.
            Default is the ``DEFAULT_DATA_DIR`` in ``RL/user_config.py``.
...
def call_experiment(exp_name, thunk, seed=0, num_cpu=1, data_dir=None, 
    ...
    Args:
        ...
        data_dir (string): Used in configuring the logger, to decide where
            ...
            default to ``DEFAULT_DATA_DIR`` from ``RL/user_config.py``. 
    ...
    plot_cmd = 'python -m RL.run plot '+logger_kwargs['output_dir']
    plot_cmd = colorize(plot_cmd, 'green')

    test_cmd = 'python -m RL.run test_policy '+logger_kwargs['output_dir']
...
class ExperimentGrid:
    ...
    def add(self, key, vals, shorthand=None, in_name=False):
        ...
        ``RL/user_config.py`` file to ``False``. 
